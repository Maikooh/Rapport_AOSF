...
## ============================================================
## CLASSIFICATION 2024 — CODE SEUL (A COLLER EN 1 FOIS)
## ============================================================
...
# Détecter automatiquement la colonne année
col_annee <- intersect(names(data), c("annee", "année", "year", "Year"))
if (length(col_annee) == 0) {
  stop("❌ Aucune colonne d'année trouvée. Colonnes disponibles : ",
...
cat("✅ Base filtrée sur", col_annee, "= 2024 | n =", nrow(data), "\n")
...
if (length(miss) > 0) stop("❌ Variables manquantes : ", paste(miss, collapse = ", "))
...
ok <- complete.cases(X)
if (!all(ok)) {
  cat("⚠️ Lignes supprimées (NA) :", sum(!ok), "\n")
  data <- data[ok, ]
  X <- X[ok, , drop = FALSE]
}
...
p_dend <- fviz_dend(hc, cex = 0.7,
                    main = "Dendrogramme — CAH (Ward) (2024)")
print(p_dend)

# Découpage CAH (modifiable)
k_cah <- 4
...
p_cah <- fviz_cluster(list(data = pca_all$x, cluster = cah_clusters),
  ...
  ggtitle(paste0("CAH Ward (k = ", k_cah, ") — Projection ACP (2024)"))
print(p_cah)
...
p_wss <- fviz_nbclust(X, kmeans, method = "wss") +
  ggtitle("Méthode du coude (WSS) — 2024")
print(p_wss)

p_sil <- fviz_nbclust(X, kmeans, method = "silhouette") +
  ggtitle("Indice de silhouette — 2024")
print(p_sil)

## ==========================================================
## C) K-means final sur toute la base 2024
## ==========================================================
k <- 4  # 🔁 change ici si tu choisis un autre k après WSS/silhouette
...
cat("\n📌 Taille des clusters (k-means, 2024)\n")
...
p_km <- fviz_cluster(list(data = res_pca$x, cluster = km$cluster),
                     geom = "point", ellipse = TRUE,
                     ggtheme = theme_minimal()) +
  ggtitle(paste0("k-means (k = ", k, ") — Projection ACP (2024)"))
print(p_km)

## ==========================================================
## D) Profils des clusters (moyennes standardisées) + barplot
## ==========================================================
res_means <- data %>%
  group_by(cluster) %>%
  summarise(across(all_of(vars_clust), mean), .groups = "drop")

cat("\n📌 Moyennes standardisées par cluster (z-scores)\n")
print(res_means)
...
p_profils <- ggplot(res_means_long, aes(x = variable, y = moyenne_z, fill = cluster)) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Profils des clusters (moyennes standardisées, 2024)",
       x = "Variable", y = "Moyenne (z-score)")
print(p_profils)

## ==========================================================
## E) Interprétation automatique (texte) des clusters
## ==========================================================
interpret_cluster <- function(means_row, seuil = 0.5) {
  ...
  list(
    hauts = if (length(hauts) == 0) "aucune variable très au-dessus de la moyenne" else paste(hauts, collapse = ", "),
    bas   = if (length(bas)   == 0) "aucune variable très en-dessous de la moyenne" else paste(bas, collapse = ", ")
  )
}
...
cat("\n==============================\n")
cat("INTERPRÉTATION — Synthèse 2024\n")
cat("==============================\n")
cat("Rappel : z > 0 = au-dessus de la moyenne nationale 2024 ; z < 0 = en-dessous.\n")
cat("Seuil utilisé : |z| ≥ ", seuil, "\n\n", sep = "")

for (cl in names(interp)) {
  cat("Cluster ", cl, "\n", sep = "")
  cat("  + Au-dessus (|z| ≥ ", seuil, ") : ", interp[[cl]]$hauts, "\n", sep = "")
  cat("  - En-dessous (|z| ≥ ", seuil, ") : ", interp[[cl]]$bas, "\n\n", sep = "")
}

cat("✅ Terminé : CAH + k-means + profils + interprétation (2024)\n")
```
